...
func runVersion(cmd *command) (err error) {
	// Check this binary matches the version in github.com/c-darwin/mobile/cmd/gomobile
	...
	version := func() string {
		...
		cmd := exec.Command("go", "install", "-x", "-n", "github.com/c-darwin/mobile/cmd/gomobile")
...
func mobileRepoRevision() (rev string, err error) {
	b, err := exec.Command("go", "list", "-f", "{{.Dir}}", "github.com/c-darwin/mobile/app").CombinedOutput()
	if err != nil {
